 ...
 * - 3 + 4 = 7을 return 합니다.
 */
const calculateNumberOfString = (myString) => {
    let calNumber = 0;
    let result = 0;
...
const calculateNumberOfStringV2 = (myString) => {
...
const calculateNumberOfStringV3 = (myString) => {
    const myStringArray = myString.split(` `)
    let resultArray = [Number(myStringArray[0])];
    for(let i = 1; i < myStringArray.length; i = i + 2) {
        const op = myStringArray[i];
        const calNumber = Number(myStringArray[i + 1]);
        if(op === "*") {
