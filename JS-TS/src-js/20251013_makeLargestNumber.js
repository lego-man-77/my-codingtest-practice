 ...
 * [0, 0, 0, 0]       "0"
 */
const makeLargestNumber = (numbers) => {
  const numbersToString = numbers.map(value => String(value));
  for(let i = 0; i < numbersToString.length; i = i + 1) {
    for(let j = i + 1; j < numbersToString.length; j = j + 1) {
      if(numbersToString[j] + numbersToString[i] > numbersToString[i] + numbersToString[j]) {
        const temp = numbersToString[j];
...
const makeLargestNumberV2 = (numbers) => {
  const numbersToString = numbers.map(value => String(value));
