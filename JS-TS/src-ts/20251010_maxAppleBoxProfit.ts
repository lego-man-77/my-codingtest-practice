 ...
 * 입출력 예 설명
 * - 예 #1: 문제의 예시와 같습니다.
 * - 예 #2: 다음과 같이 사과 상자를 포장하여 모두 팔면 최대 이익을 낼 수 있습니다.
 *   사과 상자     가격
 *   [1, 1, 2]    1 x 3 = 3
 *   [2, 2, 2]    2 x 3 = 6
 *   [4, 4, 4]    4 x 3 = 12
 ...
 */
const maxAppleBoxProfit = (k: number, m: number, score: number[]) => {
  if (score.length < m) {
    return 0;
  }
  ...
  let result = 0;
  for (let i = 0; i + m <= scoreSorted.length; i = i + m) {
    result = result + scoreSorted[i + m - 1] * m;
  }
  return result;
};

console.log(maxAppleBoxProfit(3, 4, [1, 2, 3, 1, 2, 3, 1])); // 8
console.log(maxAppleBoxProfit(4, 3, [4, 1, 2, 2, 4, 4, 4, 4, 1, 2, 4, 2])); // 33
